# Step 3: comorbidity flags (Hypertension, Diabetes, Dyslipidemia, CKD).
# Ported unchanged from CKM_Drug/Japan/scripts/03_covariates.R.
#
...
#
# Rule (design doc): a comorbidity is counted if the member has >=2
# outpatient claims OR >=1 inpatient/DPC claim with a matching ICD-10
# code, ascertained during the lookback period (same window used for
# medication flags and drug exposure).
...
comorbidity_flag_members <- function(con, icd10_codes, cfg) {
  ...
  dx_win <- diagnosis_claims(con, icd10_codes) %>%
    inner_join(win_tbl, by = "member_id") %>%
    filter(dx_date >= lookback_start, dx_date < index_date)

  outpatient_n <- dx_win %>%
    filter(type_of_claim %in% cfg$outpatient_claim_types) %>%
    distinct(member_id, claim_id) %>%
    count(member_id, name = "n_outpatient")

  admission_n <- dx_win %>%
    filter(type_of_claim %in% cfg$admission_claim_types) %>%
    distinct(member_id, claim_id) %>%
    count(member_id, name = "n_admission")

  outpatient_n %>%
    full_join(admission_n, by = "member_id") %>%
    collect() %>%
    mutate(
...
add_covariates <- function(con, cohort, cfg, log = message) {
  ...
  cohort <- cohort %>%
    mutate(
      comorbid_hypertension = member_id %in% flags$hypertension,
      comorbid_diabetes = member_id %in% flags$diabetes,
      comorbid_dyslipidemia = member_id %in% flags$dyslipidemia,
      comorbid_ckd = member_id %in% flags$ckd
    )
