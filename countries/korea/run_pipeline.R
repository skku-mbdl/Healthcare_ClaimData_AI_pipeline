...
#
#   Rscript countries/korea/run_pipeline.R            # real NHIS/CKM_DRUG run
#   Rscript countries/korea/run_pipeline.R --demo      # synthetic demo run, no DB needed
#
# --demo replaces ONLY Stage 1's real cohort-build/Cox-screen (which needs a
# live NHIS connection) with the synthetic seed data under data/synthetic/
# korea/ (see data/metadata.json and core/R/demo_data.R) -- Stage 2 then runs
...
overall_result <- tryCatch({
  ...
  stage1_log(sprintf("Stage 1 (drug screen) run %s starting (%s). Results -> %s",
                      run_id, if (demo_mode) "DEMO MODE -- synthetic data" else "real NHIS/CKM_DRUG", stage1_results_path))

  if (demo_mode) {

    stage1_cox_results <- seed_stage1_checkpoint_from_demo("korea", pipeline_root, stage1_results_path, log = stage1_log)
    stage1_log(sprintf("Stage 1 demo run %s complete (no NHIS/CKM_DRUG connection was used).", run_id))
